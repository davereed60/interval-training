    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                Text("Find the interval")
                    .font(.title2)
                    .fontWeight(.semibold)
                    .padding(.top)

                // Interval challenge display
                if let interval = viewModel.session.currentInterval,
                   let rootNote = viewModel.session.currentIntervalRootNote {
                    VStack(spacing: 12) {
                        Text(interval.rawValue)
                            .font(.title)
                            .fontWeight(.bold)
                            .foregroundColor(.orange)
                        Text("above \(rootNote.rawValue)")
                            .font(.headline)
                ...
                VStack(spacing: 15) {
                    ...
                    LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 4), spacing: 12) {
                        ForEach(Note.allCases) { note in
                            Button(action: {
                                viewModel.selectNoteForInterval(note)
                            }) {
                                Text(note.rawValue)
